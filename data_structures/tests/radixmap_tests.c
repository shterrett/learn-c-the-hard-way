...
static int test_search(RadixMap *map)
{
  ...
  for (i = map->end / 2; i < map->end; i++) {
    ...
    check(found->data.key == d->data.key,
          "Got wrong result: %p:%u looking for %u at %u",
          found,
          found->data.key,
          d->data.key,
          i
...
static char *test_operations()
{
  ...
  mu_assert(test_search(map), "Failed search test");
  ...
  while (map->end > 0) {
    ...
    mu_assert(el != NULL, "Should get result");

    size_t old_end = map->end;

    mu_assert(RadixMap_delete(map, el) == 0, "Didn't delete");
    mu_assert(old_end - 1 == map->end, "Wrong sie after delete");

    mu_assert(check_order(map), "RadixMap didn't stay sorted after delte");
  }
