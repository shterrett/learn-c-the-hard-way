...
{
  StringScanner *scan = StringScanner_create(&IN_STR, &ALPHA);
  mu_assert(scan != NULL, "Failed to make scanner");

  int find_i = String_find(&IN_STR, &ALPHA);
  mu_assert(find_i == 7, "Failed to find 'ALPHA' in test string");

  int scan_i = StringScanner_scan(scan);
  mu_assert(scan_i == 7, "Failed to find 'ALPHA' with scan");
  mu_assert(scan_i == find_i, "find and scan don't match");

  scan_i = StringScanner_scan(scan);
  mu_assert(scan_i == 18, "should find another alpha after the first");

  scan_i = StringScanner_scan(scan);
  mu_assert(scan_i == 36, "should find another alpha after the first");

  mu_assert(StringScanner_scan(scan) == -1, "no more alphas left");
...
char *test_scan_performance()
{
  ...
  do {
    for (i = 0; i < 1000; i++) {
      ...
      do {
        found_at = StringScanner_scan(scan);
        find_count++;
      } while (found_at != -1);
    }
